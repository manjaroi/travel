...
let m_month1 = m_date.getMonth() + 2;
let m_day = m_date.getDate();
let str = m_year + '-' + m_month + '-' + m_day;  
let res = m_year + '-' + m_month1 + '-' + m_day;  
...
Y.PriceCalendar = Y.extend(Calendar, Y.Base, {
    ...
    bindUI: function() {
        ...
        this._boundingBox.delegate('click', this._clickDelegate, '.next-btn, .prev-btn, .in, .out, .confirm-btn, .cancel-btn, .m_one, .m_two', this);  
        return this;
    },
    ...
    _checkout: function(oTarget) {
        ...
        if(this._toNumber(sDate) <= this._toNumber(this.get('depDate'))) {
            ...
            oTarget.addClass('m_one');
            oTarget.addClass('m_two');
        }
        oParent.one('.mark').setContent(oTarget.get('innerHTML').substr(0, 2));
        this._setConfirmStatus()._setSelectedRangeStyle();
        this.fire('checkout');
    ...
    _clickDelegate: function(e) {
        ...
        switch(true) {
            case oTarget.hasClass('prev-btn'):
                this.prevMonth();
                break;
            case oTarget.hasClass('m_one'):
                this.prevMonth();
                break;
            case oTarget.hasClass('next-btn'):
                this.nextMonth();
                break;
            case oTarget.hasClass('m_two'):
                this.nextMonth();
                break;
    ...
    _setBtnStates: function() {
        var curDate = +this._getSiblingMonth(this.get('date'), 0),
            ...
            m_one = this._boundingBox.one('.m_one'),
            m_two = this._boundingBox.one('.m_two'),
...
}, 
{
    ...
    CALENDAR_TEMPLATE: '<div id="{bounding_box_id}" class="price-calendar-bounding-box">' +
                            '<div class="container">' +
                                '<span ><a href="javascript:;" class="m_one" style="font-size:13px;color:#333;font-weight:700;border-color:#F86A5C;margin-right:20px;">'+ str +'</a></span><span ><a href="javascript:;" class="m_two style="style="font-size:13px;color:#333;font-weight:700;border-color:#F86A5C;"  ">'+ res +'</a></span>' +
